...
for (i in 1:length(nS))
{
  for (j in 1:length(es))
  {
    for (k in 1:nsim)
    {
      ...
      pL = ttL$p.value
      esL = (mean(g1L[, 1]) - mean(g2L[, 1])) / sqrt(.5 * (var(g1L[, 1]) +
                                                             var(g2L[, 1])))
      ...
      if (pL > alpha | esL < 0)
      {
        ...
        pL_2 = ttL_2$p.value
        esL_2 = (mean(g1L[, 2]) - mean(g2L[, 2])) / sqrt(.5 * (var(g1L[, 2]) +
                                                                 var(g2L[, 2])))
        ...
        if (pL_2 > alpha | esL_2 < 0)
        {
          ...
          g1L_4 = g1L
          g2L_4 = g2L
          g1L_4[which(abs((g1L_4[, 1] - mean(g1L_4[, 1])) / sd(g1L_4[, 1])) >
                        2)] = NA
          g2L_4[which(abs((g2L_4[, 1] - mean(g2L_4[, 1])) / sd(g2L_4[, 1])) >
                        2)] = NA
          g1L_4[which(abs((g1L_4[, 2] - mean(g1L_4[, 2])) / sd(g1L_4[, 2])) >
                        2)] = NA
          g2L_4[which(abs((g2L_4[, 2] - mean(g2L_4[, 2])) / sd(g2L_4[, 2])) >
                        2)] = NA
          
          ttL_4_v1 = t.test(g1L_4[, 1],
                            g2L_4[, 1],
                            var.equal = T,
                            na.rm = T)
          ...
          if ((pL_4_v1 > alpha |
               esL_4_v1 < 0) & (pL_4_v2 > alpha | esL_4_v2 < 0))
          {
            #if non is significant select the best result
            all_psL = c(pL, pL_2, pL_4_v1, pL_4_v2)
            all_esL = c(esL, esL_2, esL_4_v1, esL_4_v2)
            ...
            esL_4 = c(esL_4_v1, esL_4_v2)[which(c(pL_4_v1, pL_4_v2) ==
                                                  pL_4)]
      ...
      for (l in 1:mf)
      {
        ...
        p = tt$p.value
        esS = (mean(g1[, 1]) - mean(g2[, 1])) / sqrt(.5 * (var(g1[, 1]) +
                                                             var(g2[, 1])))
        ...
        if (p > alpha | esS < 0)
        {
          ...
          p_2 = tt_2$p.value
          es_2 = (mean(g1[, 2]) - mean(g2[, 2])) / sqrt(.5 * (var(g1[, 2]) +
                                                                var(g2[, 2])))
          ...
          if (p_2 > alpha | es_2 < 0)
          {
            g1_4 = g1
            g2_4 = g2
            g1_4[which(abs((g1_4[, 1] - mean(g1_4[, 1])) / sd(g1_4[, 1])) >
                         2)] = NA
            g2_4[which(abs((g2_4[, 1] - mean(g2_4[, 1])) / sd(g2_4[, 1])) >
                         2)] = NA
            g1_4[which(abs((g1_4[, 2] - mean(g1_4[, 2])) / sd(g1_4[, 2])) >
                         2)] = NA
            g2_4[which(abs((g2_4[, 2] - mean(g2_4[, 2])) / sd(g2_4[, 2])) >
                         2)] = NA
            
            tt_4_v1 = t.test(g1_4[, 1],
                             g2_4[, 1],
                             var.equal = T,
                             na.rm = T)
            p_4_v1 = tt_4_v1$p.value
            es_4_v1 = (mean(g1_4[, 1], na.rm = T) - mean(g2_4[, 1], na.rm =
                                                           T)) / sqrt(.5 * (var(g1_4[, 1], na.rm = T) + var(g2_4[, 1], na.rm = T)))
            
            tt_4_v2 = t.test(g1_4[, 2],
                             g2_4[, 2],
                             var.equal = T,
                             na.rm = T)
            p_4_v2 = tt_4_v2$p.value
            es_4_v2 = (mean(g1_4[, 2], na.rm = T) - mean(g2_4[, 2], na.rm =
                                                           T)) / sqrt(.5 * (var(g1_4[, 2], na.rm = T) + var(g2_4[, 2], na.rm = T)))
            
            #> If nothing helped and our hacking were ineffective
            if ((p_4_v1 > alpha |
                 es_4_v1 < 0) & (p_4_v2 > alpha | es_4_v2 < 0))
            {
              #if non is significant select the best result
              all_ps = c(p, p_2, p_4_v1, p_4_v2)
              all_es = c(esS, es_2, es_4_v1, es_4_v2)
...
for (i in 1:length(es))
{
  for (j in 1:length(nS))
  {
    for (k in 1:4)
    {
      pProp[i, (j - 1) * 4 + k] = length(which(psAr[j, i, , k][which(esAr[j, i, , k] >
                                                                       0)] < alpha)) / length(psAr[j, i, , k])
    }
  }
}
...
plot(
  es,
  apply(esAr[1, , , 1], 1, mean) - es,
  type = "l",
  col = 1,
  lty = 1,
  ylim = c(0, 1.2),
  main = paste("ES bias; Small N = ", nS[1], sep = ""),
  xlab = "ES",
  ylab = "bias"
)
lines(
  es,
  apply(esAr[1, , , 2], 1, mean) - es,
  type = "l",
  col = 2,
  lty = 1
)
...
plot(
  es,
  apply(esAr[2, , , 1], 1, mean) - es,
  type = "l",
  col = 1,
  lty = 1,
  ylim = c(0, 1.2),
  main = paste("ES bias; Small N = ", nS[2], sep = ""),
  xlab = "ES",
  ylab = "bias"
)
lines(
  es,
  apply(esAr[2, , , 2], 1, mean) - es,
  type = "l",
  col = 2,
  lty = 1
)
...
plot(
  es,
  apply(esAr[3, , , 1], 1, mean) - es,
  type = "l",
  col = 1,
  lty = 1,
  ylim = c(0, 1.2),
  main = paste("ES bias; Small N = ", nS[3], sep = ""),
  xlab = "ES",
  ylab = "bias"
)
lines(
  es,
  apply(esAr[3, , , 2], 1, mean) - es,
  type = "l",
  col = 2,
  lty = 1
)
